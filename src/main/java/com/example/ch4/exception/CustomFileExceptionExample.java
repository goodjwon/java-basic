...
public class CustomFileExceptionExample {
    public static void main(String[] args) {
        try {
            ...
            System.err.println("커스텀 예외 발생: " + e.getMessage());
        }
    }

    static void processFile(String filename) throws FileProcessingException {
        // 파일 존재하지 않거나 권한 문제 시 커스텀 예외 발생
        if (!new java.io.File(filename).exists()) {
            throw new FileProcessingException("파일 처리 불가 - 파일 존재하지 않음: " + filename);
        }
        // 실제 파일 처리 로직 가정 가능
    }
}
...
/*
코드 설명:
- FileProcessingException이라는 커스텀 예외 정의
