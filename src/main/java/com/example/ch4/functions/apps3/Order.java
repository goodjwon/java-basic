...
public class Order {
    ...
    public Order applyDiscount(double rate) {
        ...
        this.discount = totalPrice * rate;
        return this;
    }

    // 배송비 계산: 기본 배송비 + (수량당 배송비)
    public Order calculateShipping(int baseFee, int perItemFee) {
        this.shipping = baseFee + (perItemFee * quantity);
        return this;
    }

    // 포인트 적립: (총금액 - 할인 + 배송비)에 pointRate 적용
    public Order accumulatePoints(double pointRate) {
        double netPrice = (unitPrice * quantity) - discount + shipping;
        this.points = (int) (netPrice * pointRate);
        return this;
    }
